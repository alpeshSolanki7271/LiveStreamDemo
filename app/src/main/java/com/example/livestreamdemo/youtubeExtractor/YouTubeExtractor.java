package com.example.livestreamdemo.youtubeExtractor;
...
import com.example.livestreamdemo.AppConstants;
...
public abstract class YouTubeExtractor extends AsyncTask<String, Void, SparseArray<YtFile>> {
    ...
    public static String dashManifestUrl = "";
    public static String hlsManifestUrl = "";
    ...
    private SparseArray<YtFile> getStreamUrls() throws IOException, InterruptedException, JSONException {
        ...
        if (mat.find()) {
            ...
            if (streamingData.has("dashManifestUrl")) {
                dashManifestUrl = streamingData.getString("dashManifestUrl");
            }

            if (streamingData.has("hlsManifestUrl")) {
                hlsManifestUrl = streamingData.getString("hlsManifestUrl");
            }

            if (!AppConstants.isLiveVideo) {
                        ...
                        if (format.has("url")) {
                            ...
                            if (mat.find() && matSig.find()) {
                                ...
                                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.TIRAMISU) {
                                    url = URLDecoder.decode(mat.group(1), StandardCharsets.UTF_8);
                                }
                                String signature = null;
                                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.TIRAMISU) {
                                    signature = URLDecoder.decode(matSig.group(1), StandardCharsets.UTF_8);
                                }
                                ytFiles.append(itag, new YtFile(FORMAT_MAP.get(itag), url));
